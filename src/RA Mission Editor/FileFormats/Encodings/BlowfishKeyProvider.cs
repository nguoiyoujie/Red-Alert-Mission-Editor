...
namespace RA_Mission_Editor.FileFormats.Encodings
{
	/// <summary>Blowfish key provider.</summary>
	internal class BlowfishKeyProvider {
		/// <summary> The public key string </summary>
		private const string pubkeyStr = "AihRvNoIbTn85FZRYNZRcT+i6KpU+maCsEqr3Q5q+LDB5tH7Tz2qQ38V";

		private static readonly sbyte[] char2num = {
		...
		private class PublicKey {
			public readonly uint[] key1 = new uint[64];
			public readonly uint[] key2 = new uint[64];
			public uint len;
		}

		private readonly PublicKey pubkey = new PublicKey();

		private readonly uint[] glob1 = new uint[64];
		private uint glob1_bitlen, glob1_len_x2;
		private readonly uint[] glob2 = new uint[130];
		private readonly uint[] glob1_hi = new uint[4];
		private readonly uint[] glob1_hi_inv = new uint[4];
		private uint glob1_hi_bitlen;
		private uint glob1_hi_inv_lo, glob1_hi_inv_hi;

		private void init_bignum(uint[] n, uint val, uint len) {
			for (int i = 0; i < len; i++) n[i] = 0;
			n[0] = val;
		}

		private void move_key_to_big(uint[] n, byte[] key, uint klen, uint blen) {
			byte sign;
		...
		private void key_to_bignum(uint[] n, byte[] key, uint len) {
			uint keylen;
			int i;
		...
		private uint len_bignum(uint[] n, uint len) {
			uint i;
			i = len - 1;
			while ((i >= 0) && (n[i] == 0)) i--;
			return i + 1;
		...
		private uint bitlen_bignum(uint[] n, uint len) {
		...
		private uint len_predata() {
			uint a = (pubkey.len - 1)/8;
			return (55/a + 1)*(a + 1);
		}

		private int cmp_bignum(uint[] n1, uint[] n2, uint len) {
			while (len > 0) {
				--len;
		...
		private void mov_bignum(uint[] dest, uint[] src, uint len) {
			Array.Copy(src, dest, len);
		}

		private void shr_bignum(uint[] n, int bits, int len) {
			int i;
			int i2 = bits/32;
		...
		private void shl_bignum(uint[] n, int bits, int len) {
			int i, i2;
		...
		private uint sub_bignum(uint[] dest, uint[] src1, uint[] src2, uint carry, int len) {
			uint i1, i2;
		...
		private unsafe uint sub_bignum(uint* dest, uint* src1, uint* src2, uint carry, int len) {
			uint i1, i2;
		...
		private void inv_bignum(uint[] n1, uint[] n2, uint len) {
		...
		private void inc_bignum(uint[] n, uint len) {
			int i = 0;
			while ((++n[i] == 0) && (--len > 0)) i++;
		}

		private void init_two_dw(uint[] n, uint len) {
		...
		private void mul_bignum(uint[] dest, uint[] src1, uint[] src2, uint len) {
			uint i;
		...
		private void not_bignum(uint[] n, uint len) {
			uint i;
			for (i = 0; i < len; i++) n[i] = ~n[i];
		}

		private void neg_bignum(uint[] n, uint len) {
			not_bignum(n, len);
			inc_bignum(n, len);
		}
		...
		private void dec_bignum(uint[] n, uint len) {
			int i = 0;
			while ((--n[i] == 0xffffffff) && (--len > 0))
				i++;
		}

		private void calc_a_bignum(uint[] n1, uint[] n2, uint[] n3, uint len) {
		...
		private void clear_tmp_vars(uint len) {
		...
		private void calc_a_key(uint[] n1, uint[] n2, uint[] n3, uint[] n4, uint len) {
			var n_tmp = new uint[64];
			uint n3_len, n4_len;
			int n3_bitlen;
			uint bit_mask;
		...
		private unsafe void process_predata(byte* pre, uint pre_len, byte* buf) {
			var n2 = new uint[64];
			var n3 = new uint[64];

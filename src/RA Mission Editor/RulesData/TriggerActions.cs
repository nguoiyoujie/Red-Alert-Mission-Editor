  ...
  public static class TriggerActions
		...
		static TriggerActions()
    {
			...
			_listTriggerAction = new List<TriggerActionType>()
			{
				...
				new TriggerActionType("09 - Fire Sale", "The specified HOUSE sells all its structures and sets its AI to endgame mode, which constantly hunts", p3Type: TriggerParameterFlag.HOUSE),
				...
				new TriggerActionType("11 - Text Trigger (ID num)", "Displays the text MESSAGE string", p3Type: TriggerParameterFlag.MESSAGE),
				...
				new TriggerActionType("41 - Add vehicle to sidebar", "Adds VEHICLE to the trigger house's sidebar, without checking for prerequisites", p1Type: TriggerParameterFlag.UNITTYPE),
				new TriggerActionType("42 - Add infantry to sidebar", "Adds INFANTRY to the trigger house's sidebar, without checking for prerequisites", p1Type: TriggerParameterFlag.INFANTRYTYPE),
				new TriggerActionType("43 - Add structure to sidebar", "Adds INFANTRY to the trigger house's sidebar, without checking for prerequisites", p1Type: TriggerParameterFlag.STRUCTURETYPE),
				new TriggerActionType("44 - Add aircraft to sidebar", "Adds INFANTRY to the trigger house's sidebar, without checking for prerequisites", p1Type: TriggerParameterFlag.AIRCRAFTTYPE),
				new TriggerActionType("45 - Add ship to sidebar", "Adds INFANTRY to the trigger house's sidebar, without checking for prerequisites", p1Type: TriggerParameterFlag.SHIPTYPE),
				...
				new TriggerActionType("51 - Set house player control", "Set player control for a HOUSE. Use 1 to enable player control, 0 to disable", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.INTEGER),
				new TriggerActionType("52 - Set house primary color", "Set primary house COLOR for a HOUSE", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.COLOR),
				new TriggerActionType("53 - Set house secondary color", "Set secondary house COLOR for a HOUSE", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.COLOR),
				new TriggerActionType("54 - Set house tech level", "Set TECH LEVEL for a HOUSE", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.INTEGER),
				new TriggerActionType("55 - Set house IQ level", "Set secondary house COLOR for a HOUSE", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.INTEGER),
				new TriggerActionType("56 - Set house ally", "Force one HOUSE to ally another HOUSE. This works one way, so you may want to set two actions to ensure mutual alliance", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.HOUSE),
				new TriggerActionType("57 - Set house enemy", "Force one HOUSE to unally another HOUSE. This works one way, so you may want to set two actions to ensure mutual enemity (TO BE TESTED)", p1Type: TriggerParameterFlag.HOUSE, p2Type: TriggerParameterFlag.HOUSE),
				new TriggerActionType("58 - Create chrono vortex at", "Create a Chrono Vortex at a WAYPOINT", p1Type: TriggerParameterFlag.WAYPOINT),
				new TriggerActionType("59 - Nuke strike at", "Drop a nuke at a WAYPOINT", p1Type: TriggerParameterFlag.WAYPOINT),
				new TriggerActionType("60 - Capture attached objects", "Sets the HOUSE for structures, units and infantry that this trigger is attached to. Capturing structures only work for Capturable structures", p1Type: TriggerParameterFlag.HOUSE),
				new TriggerActionType("61 - Iron Curtain attached objects", "Applies the Iron Curtain effect on structures, units and infantry that this trigger is attached to. Use -1 value to use the default Iron Curtain setting, otherwise the VALUE represents the number of frames the effect lasts", p1Type: TriggerParameterFlag.INTEGER),
				new TriggerActionType("62 - Create structure at", "Create a STRUCTURE for a HOUSE at a WAYPOINT. This mimics the building placement process, so if the terrain is blocked, the structure will not be created", p1Type: TriggerParameterFlag.STRUCTURETYPE, p2Type: TriggerParameterFlag.WAYPOINT, p3Type: TriggerParameterFlag.HOUSE),
				...
				new TriggerActionType("67 - Iron Curtain trigger object", "Iron Curtains the activating object for a number of FRAMES.", p1Type: TriggerParameterFlag.INTEGER),
			};
